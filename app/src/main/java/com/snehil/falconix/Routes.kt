...
sealed class Routes(val route: String) {
    sealed class BottomNavRoutes(path: String, val label: String, val icon: ImageVector) :
        Routes(path) {
        data object HOME : BottomNavRoutes(path = "/home", label = "Home", icon = Icons.Filled.Home)
        data object SEARCH :
            BottomNavRoutes(path = "/search", label = "Search", icon = Icons.Filled.Search)
...
val HomeRoutes = arrayOf(
    Routes.BottomNavRoutes.HOME,
    Routes.BottomNavRoutes.SEARCH,
    Routes.BottomNavRoutes.STORE
)